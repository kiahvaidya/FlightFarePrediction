...
if st.button("🔍 Predict Lowest Fare"):
    ...
    if days_until_flight < 0:
        ...
        # Compact plot of booking-date vs predicted fare
        fig, ax = plt.subplots(figsize=(5, 2.5))
        ax.plot(booking_dates, predicted_fares, marker='o', linewidth=1)
        ax.set_title("Predicted fare for different booking dates")
        ax.set_xlabel("Booking date")
        ax.set_ylabel("Fare (₹)")
        ax.tick_params(axis='x', rotation=30)
        # highlight minimum
        ax.scatter([best_booking_date], [best_price], color='red', s=60, zorder=5)
        ax.annotate(f"Lowest\n₹{int(best_price)}", xy=(best_booking_date, best_price),
                    xytext=(0, -28), textcoords="offset points", ha='center', fontsize=8, bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.8))
        st.pyplot(fig)

        # also show a small table of booking dates and fares
        table = pd.DataFrame({
            "booking_date": [d.strftime("%d %b %Y") for d in booking_dates],
            "predicted_fare": [int(x) for x in predicted_fares]
        })
        st.table(table)
